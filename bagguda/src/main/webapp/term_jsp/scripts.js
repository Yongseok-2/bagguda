...
function increaseViewCount(productId) {
    // AJAX 요청
    fetch('increase_view_count.jsp?product_id=' + encodeURIComponent(productId), {
        method: 'GET'
    })
...
document.addEventListener("DOMContentLoaded", function () {
    fetch('/asd/WishlistCount')
        ...
        .then(data => {
            console.log('Wishlist count:', data.wishCount);

            // heart-count 요소 업데이트
            const heartCountElement = document.querySelector('.heart-count');
            if (heartCountElement) {
                heartCountElement.textContent = data.wishCount; // 서버로부터 받은 wishCount로 업데이트
            }
        })
        .catch(error => {
